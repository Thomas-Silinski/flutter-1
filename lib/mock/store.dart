...
typedef AllStore = Map<String, Future<OneStore> Function()>;

const String mockedNYTimesId = 'W9iqEiWfTOx';

// String _image(String name) => base64Encode(
//       File('./assets/$name').readAsBytesSync(),
//     );

Future<String> _image(String name) async {
  final ByteData bytes = await rootBundle.load('assets/mock/$name');
  final Uint8List listUint = bytes.buffer.asUint8List();
  return base64Encode(listUint);
}

Future<OneStore> _mockedArticles() async => <Data>[
      <String, String>{
        'title':
            'Higher Rents Burden Tenants, Fuel Inflation and Trouble the Fed',
        'content':
...
The rent rise may challenge that view and pressure Washington and the Federal Reserve.''',
        ...
        'thumbnail': await _image('worker.jpg'),
      },
      <String, String>{
        'title':
...
''',
        'id': 'q9EWiitEhZK',
        'author': mockedNYTimesId,
        'thumbnail': await _image('snickers.png'),
      },
      <String, String>{
        'title':
...
Employers already had the option of offering wiggle room, like allowing workers to carry some of the money in their accounts into the new year or giving grace periods of a couple of months after the deadline for spending the money. About three-fourths of accounts offered one of those options in 2019, according to the nonpartisan and nonprofit Employee Benefit Research Institute.''',
        'id': '2PBE28hfAda',
        'author': mockedNYTimesId,
        'thumbnail': await _image('money.png'),
      },
    ];

Future<OneStore> _mockedUsers() async => <Data>[
      <String, String>{
        'name': 'New York Times',
        'email': 'publisher@nytimes.com',
        'password': '5up3R-51cR3T-pA55W0RD',
        'id': mockedNYTimesId,
        'picture': await _image('nytimes.png'),
      },
    ];

final AllStore mockedStores = <String, Future<OneStore> Function()>{
  'articles': () async => await _mockedArticles(),
  'users': () async => await _mockedUsers(),
};
